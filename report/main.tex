...
\newcommand{\accIBkFive}{70.7}% TODO: re-run Weka with IBk -K 5
...
\newcommand{\fOneIBkFive}{0.439}% TODO: re-run Weka with IBk -K 5
...
\newcommand{\recIBkFive}{0.378}% TODO: re-run Weka with IBk -K 5
...
$9.5$-percentage-point improvement over our unpruned baseline. MyKNN+
...
against eleven Weka classifiers (ZeroR, OneR, IBk, NaiveBayes, J48
unpruned/pruned, MLP, SMO, Bagging, AdaBoostM1, Random Forest) and
report two notable findings: pruning a single tree is the strongest single
intervention on this dataset, and tree ensembles do \emph{not} outperform
the single pruned tree at $n=273$ with a concentrated feature signal.
We also discuss why the recall-on-\emph{died} measure matters more than
headline accuracy for medical decision support.
\end{abstract}
...
($\accJfourEightP\%$), OneR ($\accOneR\%$) and NaiveBayes ($\accNB\%$).
These five form a tight $71$--$73\%$ cluster less than two percentage
points wide. A second tier in the $67$--$70\%$ band contains MyKNN+
...
$1.5$~pp \emph{worse} than the single J48-pruned tree.
...
\begin{enumerate}[leftmargin=*,nosep,topsep=2pt]
  ...
  \item Among Weka classifiers, the linear SMO is the best clinical
        choice (highest F1 on \texttt{died}, $\fOneSMO$, with the
        second-highest accuracy of $\accSMO\%$), and the
        single J48-pruned tree out-performs every tree ensemble we
        tested. Tree ensembles therefore do \emph{not} pay their usual
        UCI-benchmark dividend on a dataset of only $273$ patients
        with concentrated feature signal: Bagging, Random Forest and
        AdaBoostM1 are all $\sim2$--$5$~pp behind their own base
        learner.
\end{enumerate}
